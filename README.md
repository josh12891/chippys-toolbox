...
**TestFlight matches the App Store:** iOS still detects TestFlight at runtime (`appStoreReceiptURL` ends with `sandboxReceipt` and there is no `embedded.mobileprovision`), but complimentary unlock is **off**. Stairs and concrete use the same freemium gate as App Store and Play (one free calculation each, then `tradies_toolbox_setout_unlock`). A real purchase or restore still writes `tradies-toolbox.unlock.v1`. We do **not** bake a Codemagic compile flag into the IPA — that binary can be promoted to the App Store.
...
- [ ] Screenshots: home, concrete, stairs, running, triangle (TestFlight shows the same paywall as the App Store after each tool’s free calculation)
