...
1. Install the new TestFlight build (marketing version **1.0.5** after this lands on `main`). Codemagic sets `CFBundleVersion` to the latest TestFlight build number plus one. Delete the 1.0.4 TestFlight install first if it is still on the phone — that build showed `$5.99` on Unlock set-out while the purchase sheet showed `A$9.99`.
